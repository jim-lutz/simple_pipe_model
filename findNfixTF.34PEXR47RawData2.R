...
# this is code find and fix anomalies in TestFlag in DT_data.3 for 34PEXR47RawData2
...
# ===
timestamp.data <- 
  DT_data.3[, list(start = min(timestamp, na.rm = TRUE),
...
# find
# ---
# report if TestFlag contains '.' and not 'R-4.'
if(nrow(DT_data.3[grepl("\\.",TestFlag) &
                    !grepl("R-4\\.",TestFlag), 
                  list(timestamp, record, TestFlag)])>0) {
...
DT_data.3[grepl("FIXTURE",TestFlag),
          list(timestamp, record, TestFlag)]
#           timestamp record      TestFlag
# 2009-02-22 15:05:29 242475   PEX FIXTURE

# TestFlag == 'PEX FIXTURE', probably should be 'OTHER FIXTURE'
DT_data.3[TestFlag == 'PEX FIXTURE', TestFlag:= 'OTHER FIXTURE']
...
# insulation level
# ===
DT_data.3[grepl("BARE|R-5|R-4",TestFlag), 
          list(n=length(record)), by=TestFlag]
# there's some typos, R-4.6 instead of R-4.7
DT_data.3[TestFlag=='R-4.6', TestFlag:= 'R-4.7']

DT_data.3[grepl("BARE|R-5|R-4",TestFlag) & is.na(insul.level), 
          insul.level := str_match(TestFlag, "(BARE|R-5.2|R-4.7|R-5.5)")[2]]
